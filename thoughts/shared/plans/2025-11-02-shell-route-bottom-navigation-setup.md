...
**Strategy Note**: We'll implement routing and screens first using `package:kairos/` imports, then rename the actual package metadata as the final step. This prevents import breakage during development.
...
A Flutter app with:
1. GoRouter ShellRoute containing all authenticated routes
2. Persistent MainScaffold with BottomNavigationBar (4 tabs)
3. Tab navigation that preserves state and doesn't rebuild unnecessarily
4. Profile onboarding flow: Auth → Profile Creation → Main App
5. Placeholder screens for Journal and Notifications
6. Home tab replacing the current Dashboard
7. Package name changed from `blueprint_app` to `kairos` across all files
...
- Tab switching is smooth without rebuilding scaffold or tab content
...
- Firebase still connects properly after package rename
...
- Adding custom animations or transitions (using GoRouter defaults)
- Implementing deep linking or URL strategies
- Adding tablet/desktop adaptive layouts
- Implementing state restoration for tabs
- Adding nested navigation within tabs (may add later via TODO comments)

## Implementation Approach

This will be a 5-phase implementation:
1. **Route Structure Setup**: Define routes, create MainScaffold, configure ShellRoute with loading screen
2. **Tab Screens Creation**: Build Home, Journal, Notifications, Profile screens (NO nested Scaffolds)
3. **Navigation State Management**: Wire up bottom nav with routing
4. **Onboarding Flow Integration**: Update redirects for profile creation flow
5. **Package Renaming & Cleanup**: Update all references from blueprint_app to kairos (FINAL STEP)

**Why this order?** Implementing routing first and renaming last prevents import breakage during development. We'll use `package:kairos/` in code but keep `blueprint_app` package name until everything works.
...
### Overview
Define new route constants, create a loading/splash screen, create the MainScaffold widget with BottomNavigationBar, and configure GoRouter's ShellRoute to wrap all authenticated routes with the persistent scaffold. Extract redirect logic into a helper function for maintainability.
...
#### 2. Create Loading/Splash Screen
**File**: `lib/core/routing/pages/loading_page.dart` (NEW FILE)
**Changes**: Create a simple loading screen to show during auth initialization
...
#### 4. Create Auth Redirect Helper Function
**File**: `lib/core/routing/auth_redirect.dart` (NEW FILE)
**Changes**: Extract redirect logic for better readability and testability
...
#### 5. Update Router Configuration with ShellRoute
**File**: `lib/core/routing/router_provider.dart`
**Changes**: Replace flat routes with ShellRoute structure, add loading screen, use redirect helper
...
- [x] No compilation errors: `flutter analyze` (will show errors for missing screen files - that's expected)
- [x] Router file compiles without syntax errors

#### Manual Verification:
- [ ] App shows loading screen during auth initialization
- [ ] Login flow redirects correctly (cannot test fully until Phase 2)
- [ ] Bottom navigation structure is defined (cannot see it until screens exist)

**Implementation Note**: After completing this phase, the app won't run yet because screen files don't exist. That's expected! Move directly to Phase 2 to create the screens.

---

## Phase 2: Tab Screens Creation

### Overview
Create the four main tab screens: Home (migrating Dashboard content), Journal (placeholder), Notifications (placeholder), and Profile (placeholder). **Important**: Do NOT wrap these in Scaffold - MainScaffold already provides the scaffold wrapper.
...
**Changes**: Create home screen by migrating dashboard content. **Note**: Returns Column with AppBar, not Scaffold.
...
- [ ] **STATE PRESERVATION TEST**: Switch to Journal, then Home, then back to Journal - verify Journal doesn't rebuild (add debug print to test)
- [ ] No double app bars (would indicate nested Scaffold issue)

**Implementation Note**: After completing this phase and all automated verification passes, pause here for manual confirmation that all screens display correctly, navigation works, and tab state is preserved before proceeding to Phase 3.

---

## Phase 3: Profile Creation Integration & Flow Testing
...
#### 1. Update Create Profile Screen with Navigation
...
**Implementation Note**: After completing this phase and all automated verification passes, pause here for manual confirmation that the entire onboarding flow works correctly before proceeding to Phase 4.

---

## Phase 4: Update Main App Title

### Overview
Update the app title from "Blueprint App" to "Kairos" in main.dart as a quick win before the full package rename.

### Changes Required:

#### 1. Update Main App Title
**File**: `lib/main.dart`
...
#### Manual Verification:
- [ ] App title shows "Kairos" in recent apps / app switcher
- [ ] No other regressions

**Implementation Note**: This is a quick change. Verify it works and move to Phase 5 for the full package rename.

---

## Phase 5: Package Renaming & Final Cleanup

### Overview
Update the package name from `blueprint_app` to `kairos` across all Dart files, platform-specific configurations, and build files. Remove old dashboard route and perform final cleanup.

**IMPORTANT**: This phase changes package identifiers, which may require Firebase config updates. See the Firebase verification step below.
...
**Method**: Use global find-and-replace in your IDE:
- Find: `package:blueprint_app/`
- Replace: `package:kairos/`
- Scope: `lib/` and `test/` directories
...
- [ ] **FIREBASE CONFIG CHECK**: Verify Firebase still connects properly
  - If Firebase fails to connect, you may need to re-download config files:
    - Android: `google-services.json` (update with new package name in Firebase Console)
    - iOS: `GoogleService-Info.plist` (update with new bundle ID in Firebase Console)
...
- `authRedirectLogic` helper function tests (isolated redirect logic)
...
5. **State Preservation Test**:
   - Add debug print statements in each tab screen's build method
   - Switch between tabs multiple times
   - Verify that tab screens only build once (state is preserved)
   - If screens rebuild on every switch, investigate ShellRoute configuration
...
- **State Preservation**: Each tab screen maintains its own state when switching between tabs (verify with debug prints)
- **Image Loading**: Profile avatars use `NetworkImage` with implicit caching (consider adding `cached_network_image` in future for better performance)
- **Loading Screen**: Shows immediately during auth initialization to prevent blank screen flash
...
**Firebase Config**:
- After package rename, Firebase config files may need updating
- Check Firebase Console for new package name / bundle ID
- Re-download `google-services.json` (Android) and `GoogleService-Info.plist` (iOS) if needed
...
│   │   ├── auth_redirect.dart (NEW - extracted redirect logic)
│   │   ├── router_provider.dart (UPDATED)
│   │   └── pages/
│   │       ├── error_page.dart
│   │       ├── loading_page.dart (NEW)
